...
function createUser() {
	...
	$pwd = random_string();
	$sql = "INSERT INTO tbl_users (name, pwd, address, city, province, zip, notes, phone, email, type, status, bdate)
			VALUES ('$name', '$pwd', '$address', '$city', '$province', '$zip', '$notes', '$phone', '$email', '$type', 'active', NOW())";	
	dbQuery($sql);
	...
	//send_email($data);
	header('Location: ../views/?v=USERS&msg=' . urlencode('Successfully registered user.'));
	exit();
}
